...
TOOLS_PROMPT = """
--- ОСОБЕННОСТИ ОКРУЖЕНИЯ (WSL) ---
Бэкенд работает внутри WSL с графической подсистемой (Xvfb на DISPLAY=:99).
- Для выполнения консольных команд внутри этого окружения (установка пакетов, проверка логов) используйте `shell_tool`, добавляя префикс: `wsl -e bash -c "ваша команда"`.
- Буфер обмена может быть изолирован от Windows, поэтому `type_text_at` эмулирует посимвольный ввод.
- Сочетания клавиш могут не работать
"""
