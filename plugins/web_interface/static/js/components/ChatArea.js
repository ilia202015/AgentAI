        ...
        <div class="group flex flex-col w-[95%] mx-auto animate-fade-in-up" :class="msg.role === 'user' ? 'items-end' : 'items-start'">
            ...
            <div v-else class="flex flex-col w-full gap-1">
                <div v-for="(item, idx) in processedTimeline" :key="idx" class="w-full flex flex-col" :class="msg.role === 'user' ? 'items-end' : 'items-start'">
                    ...
                    <div v-else-if="item.type === 'pair'" class="w-full my-1"><details class="group/tools"><summary class="list-none cursor-pointer flex items-center gap-2 text-xs text-gray-500 hover:text-gray-300 transition-colors py-1 select-none"><i class="ph ph-wrench text-emerald-500 group-open/tools:rotate-180 transition-transform"></i><span>Использован инструмент: {{ item.request.title ? item.request.title.replace('Запрос ', '') : 'Unknown' }}</span><span class="opacity-50 text-[10px] ml-auto">Развернуть</span></summary><div class="mt-2 pt-2 border-t border-white/5"><div class="rounded-t-lg border border-white/10 bg-[#1e222a]/80 backdrop-blur overflow-hidden relative group/req"><div class="px-3 py-1.5 bg-white/5 border-b border-white/5 text-xs text-emerald-400 font-mono flex justify-between items-center"><span>{{ item.request.title }} <span class="text-[10px] text-gray-500 font-normal ml-2" v-html="formatToolMetrics(item.request._msgMetrics || msg.metrics, 'output')"></span></span><button @click="copyToClipboard(item.request.content, $event)" class="text-gray-500 hover:text-white opacity-0 group-hover/req:opacity-100 transition-opacity"><i class="ph ph-copy"></i></button></div><div class="p-3 overflow-x-auto font-mono text-xs text-gray-300 whitespace-pre-wrap no-ligatures">{{ item.request.content }}</div></div><div class="rounded-b-lg border-x border-b border-white/10 bg-[#1e222a]/80 backdrop-blur overflow-hidden relative group/res"><div class="px-3 py-1.5 bg-white/5 border-y border-white/5 text-xs text-blue-400 font-mono flex justify-between items-center"><span>Результат <span class="text-[10px] text-gray-500 font-normal ml-2" v-html="formatToolMetrics(item.result._msgMetrics || msg.metrics, 'input')"></span></span><button @click="copyToClipboard(item.result.content, $event)" class="text-gray-500 hover:text-white opacity-0 group-hover/res:opacity-100 transition-opacity"><i class="ph ph-copy"></i></button></div><div class="p-3 overflow-x-auto font-mono text-xs text-gray-400 whitespace-pre-wrap no-ligatures">{{ item.result.content }}</div></div></div></details></div>
                    <div v-else-if="item.type === 'tool'" class="w-full my-1"><details class="group/tools"><summary class="list-none cursor-pointer flex items-center gap-2 text-xs text-gray-500 hover:text-gray-300 transition-colors py-1 select-none"><i class="ph ph-wrench text-emerald-500 group-open/tools:rotate-180 transition-transform"></i><span>Инструмент: {{ item.title }}</span><span class="opacity-50 text-[10px] ml-auto">Развернуть</span></summary><div class="mt-2 pt-2 border-t border-white/5"><div class="rounded-lg border border-white/10 bg-[#1e222a]/80 backdrop-blur overflow-hidden relative group/tool"><div class="px-3 py-1.5 bg-white/5 border-b border-white/5 text-xs text-emerald-400 font-mono flex justify-between items-center"><span>{{ item.title }} <span class="text-[10px] text-gray-500 font-normal ml-2" v-html="formatToolMetrics(item._msgMetrics || msg.metrics, item.title && item.title.startsWith('Результат') ? 'input' : 'output')"></span></span><button @click="copyToClipboard(item.content, $event)" class="text-gray-500 hover:text-white opacity-0 group-hover/tool:opacity-100 transition-opacity"><i class="ph ph-copy"></i></button></div><div class="p-3 overflow-x-auto font-mono text-xs text-gray-300 whitespace-pre-wrap no-ligatures">{{ item.content }}</div></div></div></details></div>
    ...
    setup(props, { emit }) {
        ...
        const copyToClipboard = (text, event) => {
            ...
            if (type === 'output' && metrics.output_tokens !== undefined) {
                ...
                return `(Выход: ${metrics.output_tokens} токенов${time})`;
            }
            if (type === 'input' && metrics.input_tokens !== undefined) {
                const time = metrics.input_time ? ` • ${metrics.input_time.toFixed(2)} сек` : '';
                const load = metrics.uncached_tokens !== undefined ? ` (${metrics.uncached_tokens} лоад)` : '';
                const cache = metrics.cached_tokens !== undefined ? ` (+${metrics.cached_tokens} кеш)` : '';
                return `(Вход: ${metrics.total_context || metrics.input_tokens}${load}${cache} токенов${time})`;
            }
            return '';
        };
