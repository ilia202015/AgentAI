...
renderer.code = function(code, language) {
  ...
  try {
    ...
    let highlighted = safeCode.replace(/&/g,"&amp;").replace(/</g,"&lt;").replace(/>/g,"&gt;");
    ...
    if (window.hljs) {
      try {
        ...
        if (validLang) highlighted = hljs.highlight(safeCode, { language: lang }).value;
         ...
         // fallback if hljs fails on weird input
      }
    }
    ...
    <div class="group flex flex-col w-[95%] mx-auto" :class="[msg.role === 'user' ? 'items-end' : 'items-start', { 'msg-animate-appear': isAnimating }]">
      ...
      <div v-else class="flex flex-col w-full gap-1">
        <div v-for="(item, idx) in processedTimeline" :key="idx" class="w-full flex flex-col" :class="msg.role === 'user' ? 'items-end' : 'items-start'">
          <div v-if="item.type === 'thought'" class="w-full"><details class="group/thought"><summary class="list-none cursor-pointer flex items-center gap-2 text-xs text-gray-500 hover:text-gray-300 transition-colors py-1 select-none"><i class="ph ph-brain text-purple-400 group-open/thought:rotate-180 transition-transform"></i><span>Процесс мышления</span><span class="opacity-50 text-[10px] ml-auto">Развернуть</span></summary><div class="mt-2 pl-3 border-l-2 border-purple-500/20 text-gray-400 text-xs leading-relaxed font-mono p-4 border border-white/5 rounded-r-lg relative group/content" :class="store.blurClass" :style="{ backgroundColor: store.dimmingBgColorLight }"><button @click="copyToClipboard(item.content, $event)" class="absolute top-2 right-2 opacity-0 group-hover/content:opacity-100 transition-opacity p-1 text-gray-500 hover:text-white" title="Копировать"><i class="ph ph-copy"></i></button><div class="font-bold text-purple-400 mb-1">Thought Process:</div><MarkdownContent :content="item.content" /></div></details></div>
          <div v-else-if="item.type === 'pair'" class="w-full my-1"><details class="group/tools"><summary class="list-none cursor-pointer flex items-center gap-2 text-xs text-gray-500 hover:text-gray-300 transition-colors py-1 select-none"><i class="ph ph-wrench text-emerald-500 group-open/tools:rotate-180 transition-transform"></i><span>Использован инструмент: {{ item.request.title ? item.request.title.replace('Запрос ', '') : 'Unknown' }}</span><span class="opacity-50 text-[10px] ml-auto">Развернуть</span></summary><div class="mt-2 pt-2 border-t border-white/5"><div class="rounded-t-lg border border-white/10 overflow-hidden relative group/req" :class="store.blurClass" :style="{ backgroundColor: store.dimmingBgColorLight }"><div class="px-3 py-1.5 bg-white/5 border-b border-white/5 text-xs text-emerald-400 font-mono flex justify-between items-center"><span>{{ item.request.title }} <span v-html="formatToolMetrics(item.request._msgMetrics, 'output')"></span></span><button @click="copyToClipboard(item.request.content, $event)" class="text-gray-500 hover:text-white opacity-0 group-hover/req:opacity-100 transition-opacity"><i class="ph ph-copy"></i></button></div><div class="p-3 overflow-x-auto font-mono text-xs text-gray-300 whitespace-pre-wrap no-ligatures">{{ item.request.content }}</div></div><div class="rounded-b-lg border-x border-b border-white/10 overflow-hidden relative group/res" :class="store.blurClass" :style="{ backgroundColor: store.dimmingBgColorLight }"><div class="px-3 py-1.5 bg-white/5 border-y border-white/5 text-xs text-blue-400 font-mono flex justify-between items-center"><span>Результат <span v-html="formatToolMetrics(item.result._msgMetrics, 'input')"></span></span><button @click="copyToClipboard(item.result.content, $event)" class="text-gray-500 hover:text-white opacity-0 group-hover/res:opacity-100 transition-opacity"><i class="ph ph-copy"></i></button></div><div class="p-3 overflow-x-auto font-mono text-xs text-gray-400 whitespace-pre-wrap no-ligatures">{{ item.result.content }}</div></div></div></details></div>
          <div v-else-if="item.type === 'tool'" class="w-full my-1"><details class="group/tools"><summary class="list-none cursor-pointer flex items-center gap-2 text-xs text-gray-500 hover:text-gray-300 transition-colors py-1 select-none"><i class="ph ph-wrench text-emerald-500 group-open/tools:rotate-180 transition-transform"></i><span>Инструмент: {{ item.title }}</span><span class="opacity-50 text-[10px] ml-auto">Развернуть</span></summary><div class="mt-2 pt-2 border-t border-white/5"><div class="rounded-lg border border-white/10 overflow-hidden relative group/tool" :class="store.blurClass" :style="{ backgroundColor: store.dimmingBgColorLight }"><div class="px-3 py-1.5 bg-white/5 border-b border-white/5 text-xs text-emerald-400 font-mono flex justify-between items-center"><span>{{ item.title }} <span v-html="formatToolMetrics(item._msgMetrics, item.title && item.title.startsWith('Результат') ? 'input' : 'output')"></span></span><button @click="copyToClipboard(item.content, $event)" class="text-gray-500 hover:text-white opacity-0 group-hover/tool:opacity-100 transition-opacity"><i class="ph ph-copy"></i></button></div><div class="p-3 overflow-x-auto font-mono text-xs text-gray-300 whitespace-pre-wrap no-ligatures">{{ item.content }}</div></div></div></details></div>
          ...
          <div v-else-if="item.type === 'text' && (item.content.trim() || isEditing)" class="relative max-w-full overflow-hidden transition-all shadow-lg w-full group/text" :class="[[msg.role === 'user' ? 'text-white rounded-2xl rounded-tr-sm px-5 py-3.5 border border-white/10 w-auto self-end' : ' border border-white/5 text-gray-100 rounded-2xl rounded-tl-sm px-6 py-5'], store.blurClass]" :style="msg.role === 'user' ? { background: 'linear-gradient(to bottom right, rgba(37, 99, 235, ' + (store.uiDimmingPercent / 100) + '), rgba(79, 70, 229, ' + (store.uiDimmingPercent / 100) + '))' } : { backgroundColor: store.dimmingBgColor }"><div v-if="msg.role === 'user'" class="whitespace-pre-wrap font-sans text-sm leading-relaxed">{{ item.content }}</div><MarkdownContent v-else :content="item.content" /><div v-if="msg.role === 'assistant' || msg.role === 'model'" class="absolute top-2 right-2 opacity-0 group-hover/text:opacity-100 transition-opacity flex gap-1"><button @click="copyToClipboard(item.content, $event)" class="p-1.5 text-gray-500 hover:text-white hover:bg-white/10 rounded transition" title="Копировать"><i class="ph ph-copy"></i></button></div></div>
        </div>
        <!-- Метрики сообщения -->
        <div v-if="msg.metrics && (msg.metrics.input_tokens || msg.metrics.output_tokens)" class="flex mt-1 text-[10px] opacity-50 font-mono tracking-wide" :class="msg.role === 'user' ? 'justify-end' : 'justify-start'">
          <div v-if="msg.role === 'user' && msg.metrics.input_tokens" class="flex items-center gap-1.5">
            <i class="ph ph-arrow-circle-up"></i> Вход: {{ msg.metrics.input_tokens }} 
            <span v-if="msg.metrics.uncached_tokens !== undefined" class="text-blue-400 opacity-80" title="Прочитано без кеша (API Load)">({{ msg.metrics.uncached_tokens }} лоад)</span>
            <span v-if="msg.metrics.cached_tokens" class="text-purple-400 opacity-80" title="Кешировано контекста">(+{{ msg.metrics.cached_tokens }} кеш)</span>
            токенов <span class="mx-0.5">•</span> {{ msg.metrics.input_time ? msg.metrics.input_time.toFixed(2) : 0 }} сек
          </div>
  ...
  setup(props, { emit }) {
    ...
    const copyToClipboard = (text, event) => {
      ...
      if (type === 'input' && metrics.input_tokens !== undefined) {
        const time = metrics.input_time ? ` • ${metrics.input_time.toFixed(2)} сек` : '';
        return `<span class="text-[10px] text-gray-500 font-normal ml-2 tracking-wide opacity-80">(Вход: +${metrics.input_tokens} токенов${time})</span>`;
      }
      return '';
    };
  ...
  template: `
    <div class="flex-1 flex flex-col h-full relative z-10 min-w-0 transition-all duration-300 ease-in-out" @dragover.prevent="onDragOver" @dragenter.prevent="onDragOver">
      <div class="hidden md:block absolute top-4 left-2 z-50">
        <button @click="store.toggleSidebarDesktop()" class="p-2 rounded-lg bg-gray-900/50 border border-white/10 text-gray-400 hover:text-white transition-colors shadow-sm" :class="store.blurClass">
      ...
      <div class="absolute top-4 right-4 z-50 flex gap-2">
        <button @click="store.isPromptPanelOpen = !store.isPromptPanelOpen" 
            class="p-2 rounded-lg bg-gray-900/50 border border-white/10 text-gray-400 hover:text-blue-400 transition-all shadow-sm" :class="[store.blurClass, store.isPromptPanelOpen ? 'text-blue-400 border-blue-500/30 bg-blue-500/10' : '']"
      ...
      <div class="md:hidden absolute top-4 left-4 z-50">
        <button @click="store.toggleSidebarMobile()" class="p-2 rounded-lg border border-white/10 text-gray-300" :class="store.blurClass">
      ...
      <div class="absolute bottom-32 right-8 z-40 transition-all duration-300"
        ...
        <button @click="scrollToBottom(true)" 
          class="w-10 h-10 rounded-full bg-gray-800/80 border border-white/10 shadow-xl flex items-center justify-center text-gray-400 hover:text-white hover:bg-gray-700 transition-colors cursor-pointer" :class="store.blurClass">
      ...
      <div class="flex-1 overflow-y-auto px-2 md:px-0 pt-16 md:pt-6 pb-48 space-y-8 scroll-smooth custom-scrollbar" 
        ...
        <div v-if="filteredMessages.length > 0 && (store.totalInputTokens > 0 || store.totalOutputTokens > 0)" 
          ...
          <div class="border border-white/5 rounded-xl p-3 flex flex-col items-center justify-center text-center shadow-inner" :class="store.blurClass">
          ...
          <div class="border border-white/5 rounded-xl p-3 flex flex-col items-center justify-center text-center shadow-inner" :class="store.blurClass" title="Суммарная нагрузка на API без учета кеша">
          ...
          <div v-if="store.totalCachedTokens > 0" class="border border-white/5 rounded-xl p-3 flex flex-col items-center justify-center text-center shadow-inner" :class="store.blurClass">
      ...
      <div v-if="isDragging" 
         ...
         @dragover.prevent
         class="absolute inset-0 z-50 flex items-center justify-center m-2 rounded-xl border-2 border-dashed border-blue-500" :class="store.blurClass">
      ...
      for (const msg of raw) {
        ...
        if (msg.role === 'user' && last && isAgent(last.role)) {
          if (!hasText && !hasTextItems && !hasImages && hasTools) {
            ...
            const toolsToAdd = msg.items.filter(i => i.type === 'tool').map(i => ({...i, _msgMetrics: msg.metrics}));
            last.items.push(...toolsToAdd);
            continue;
          }
        }
        ...
        if (last && isAgent(last.role) && isAgent(msg.role)) {
          ...
          if (msg.metrics) {
            if (!last.metrics) last.metrics = { input_tokens: 0, output_tokens: 0, output_time: 0 };
            if (msg.metrics.output_tokens) last.metrics.output_tokens = (last.metrics.output_tokens || 0) + msg.metrics.output_tokens;
            if (msg.metrics.output_time) last.metrics.output_time = (last.metrics.output_time || 0) + msg.metrics.output_time;
            if (msg.metrics.input_tokens && !last.metrics.input_tokens) last.metrics.input_tokens = msg.metrics.input_tokens;
          }
          if (msg.content) last.content = (last.content ? last.content + '\\n' : '') + msg.content;

        } else {
          const clonedMsg = { ...msg, images: msg.images ? [...msg.images] : undefined };
          if (msg.items) {
            clonedMsg.items = msg.items.map(i => ({...i, _msgMetrics: i._msgMetrics || msg.metrics}));
          }
          if (msg.parts) {
            clonedMsg.parts = [...msg.parts];
          }
          if (msg.metrics) {
            clonedMsg.metrics = { ...msg.metrics };
          }
          merged.push(clonedMsg);
        }
      }
      return merged;
    });
