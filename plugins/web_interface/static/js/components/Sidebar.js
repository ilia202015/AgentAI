    ...
    <div>
      ...
      <Teleport to="body">
        <div v-if="store.isSidebarOpenMobile" 
           class="fixed inset-0 z-[60] md:hidden transition-all duration-300" :class="store.blurClass"
           @click="store.closeSidebarMobile()"></div>
      </Teleport>

      <!-- Sidebar Container -->
      <div class="fixed md:relative inset-y-0 left-0 border-r border-white/5 flex flex-col h-full flex-shrink-0 z-[70] transition-all duration-300 ease-in-out overflow-hidden" :style="{ backgroundColor: store.dimmingBgColor }" 
         :class="[
        ...
        <!-- Изолированный фоновый блюр -->
        <div class="absolute inset-0 pointer-events-none z-0" :class="store.blurClass"></div>
        
        <div class="flex flex-col h-full w-[280px] relative z-10"> 
          ...
          <div class="p-5 flex justify-between items-center text-gray-100">
            ...
            <button @click="store.toggleSettingsModal()" class="p-1.5 rounded-lg hover:bg-white/10 transition-colors text-gray-500 hover:text-blue-400" 
          ...
          <!-- List -->
          <div class="flex-1 overflow-y-auto px-3 py-2 space-y-1 custom-scrollbar relative z-[10]">
            ...
            >
              ...
              <div class="absolute right-2 top-1/2 -translate-y-1/2 transition-all duration-200 flex gap-1 rounded-lg p-1 shadow-xl border border-white/10" :class="[store.blurClass, [editingId === chat.id ? 'hidden' : '', activePresetMenuId === chat.id ? 'opacity-100' : 'opacity-0 group-hover:opacity-100']]">
          ...
          <!-- Footer -->
          <div class="p-4 border-t border-white/5 bg-black/20 space-y-3 relative z-[60]">
